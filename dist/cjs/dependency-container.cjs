...
class InternalDependencyContainer {
    ...
    resolve(token, context = new resolution_context_1.default()) {
        ...
        if (!registration && (0, providers_1.isNormalToken)(token)) {
    ...
    resolveAll(token, context = new resolution_context_1.default()) {
        ...
        if (!registrations && (0, providers_1.isNormalToken)(token)) {
            throw new Error(`Attempted to resolve unregistered dependency token: "${token.toString()}"`);
        }
    ...
    clearInstances() {
        ...
        for (const [token, registrations] of this._registry.entries()) {
            this._registry.setAll(token, registrations
                .filter(registration => !(0, providers_1.isValueProvider)(registration.provider))
                .map(registration => {
